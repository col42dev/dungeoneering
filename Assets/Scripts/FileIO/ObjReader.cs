			...
			while (index < fileLines.Length) {
				...
				// Read vertices
					...
					if (lineInfo.Length != 4) {
						...
						// Invert x value so it works properly in Unity (left-handed)  //cmoore removed
				...
				// Read normals
					...
					if (lineInfo.Length != 4) {
						...
						// Invert x value so it works properly in Unity //cmoore removed
		...
		for (int i = 0; i < numberOfGroupsUsed; i++) {
			if (!useSubmesh || (useSubmesh && i == 0)) {
				...
				for (int j = triStart; j < triEnd; j += 3) {
					...
					//newTriangles[counter+1] = vertHash[triVerts[j+2]]; //cmoore removed
					//newTriangles[counter+2] = vertHash[triVerts[j+1]];  //cmoore removed

					newTriangles[counter+1] = vertHash[triVerts[j+1]];
					newTriangles[counter+2] = vertHash[triVerts[j+2]];
